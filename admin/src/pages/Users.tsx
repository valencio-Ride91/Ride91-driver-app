...
import { api, AdminUserRow } from "../api";
import { AdminIdentity } from "../auth";

const ROLES = ["viewer", "manager", "owner"] as const;
const ROLE_HELP = "viewer = read-only · manager = ops actions · owner = accounts + settings";
...
export default function Users({ admin }: { admin: AdminIdentity }) {
  ...
  const load = useCallback(async () => {
    try {
      const r = await api.get<{ items: AdminUserRow[] }>("/admin/users");
      setRows(r.items);
    } finally {
      setLoading(false);
    }
  ...
  const create = async () => {
    ...
    setSaving(true);
    try {
      await api.post("/admin/users", { username: uName.trim(), password: uPw, role: uRole });
      setUName(""); setUPw(""); setURole("manager"); setAdding(false);
      await load();
      flash("Account created.");
    } catch (e: any) {
      setErr(e?.body?.detail === "username_taken" ? "That username is taken." : "Could not create account.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const setRole = async (u: AdminUserRow, role: string) => {
    try { await api.patch(`/admin/users/${u.id}`, { role }); await load(); flash(`${u.username} → ${role}.`); }
